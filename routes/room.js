...
router.get("/defaultOccupancyPattern",middleware.authenticate,function(req,res){
...
middleware.authenticate,function(req,res){
  ...
  try{
    ...
    let guid = uuidv4()
   let query = `Insert into UserConfiguration values( '', '${req.body.bedrooms}', '${req.body.roomSize}',
   '${JSON.stringify(req.body.room)}', '${new Date().toISOString()}', '${new Date().toISOString()}',
    ...
    request.query(query, function (err,set) { 
      if(err){
        ...
        rooms.forEach(element=>{
          promise.push(addRoom(element,req.decoded.id,set.recordset[0].id,req.body).then(resultAddRoom=>{
...
router.get('/configuration/:id',middleware.authenticate,function(req,res){
  ...
  request.query(query, function (err,set) { 
    if(err){
     ...
     console.log("set.recordsets[0][0].roomNames",set.recordsets[0][0].roomNames)
     set.recordsets[0][0].roomNames = JSON.parse(set.recordsets[0][0].roomNames)
        ...
        if(err){
          ...
          request.query(query, function (err,set) { 
            if(err){
              ...
              res.json({
                  success:true,
                  message:"All the rooms, occupancy details and configuration details",
                  result: resultArray
                })
...
middleware.authenticate,function(req,res){
  ...
  try{
    ...
    let guid = uuidv4()
   let query = `update UserConfiguration Set configurationName = '${req.body.configurationName}', updatedOn = '${new Date().toISOString()}' Where Id = ${req.params.id}`
    ...
    request.query(query, function (err,set) { 
      if(err){
        ...
        let promise = []
        rooms.forEach(element=>{
          promise.push(updateRoom(element,req.decoded.id,req.params.id).then(resultAddRoom=>{
            console.log("Add room res",resultAddRoom)
          }).catch(err=>{
            console.log("err",err)
          })
...
router.put('/occupancyPattern/:OPid',
...
check('occupancyPatternHoursMinute').exists()],middleware.authenticate,function(req,res){
  ...
  occupancyPatternHoursMinute  = '${req.body.occupancyPatternHoursMinute}' Where Id = ${req.params.OPid}`
...
function addRoom(roomName,id,guid,roomData){
  ...
  return new Promise((resolve,reject)=>{
    ...
    '${roomData.currentRating}', '${Number(roomData.roomSize/roomData.room.length).toFixed(2)}', 
...
function updateRoom(roomData,userId,configID){
  ...
  return new Promise((resolve,reject)=>{
    ...
    if(!roomData.Id || roomData.Id == null){
      ...
      '${configID}', '${roomData.ceilingHeightFeet}') SELECT SCOPE_IDENTITY() as id`
      console.log("query",query)
      request.query(query, function (err,set) { 
        if(err){
