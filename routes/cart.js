...
router.get('/cartDetails', middleware.authenticate, (req, res) => {
  try {
              ...
              if (err) {
                ...
                request.query(query3, function (err, set) {
                  if (err) {
                    ...
                    resultArray.push({ InstallationCharges: set.recordsets[0] })
                    let charges = {
...
router.get('/getProductDetails/:ID', middleware.authenticate, (req, res, err) => {
  try {
    validationResult(req).throw()
      ...
      request.query(query, function (err, set) {
        if (err) {
          ...
          if (set.recordsets[0].length) {
            let query = `update DeliveryAddress Set name = '${req.body.name}', contactNumber = '${req.body.contactNumber}', email = '${req.body.email}',
              address = '${req.body.address}', updatedOn = '${new Date().toISOString()}' Where userId = '${req.decoded.id}' `
            ...
            let query = `Insert into DeliveryAddress values( '${req.body.name}', '${req.body.contactNumber}', '${req.body.email}',
              '${req.body.address}', '${req.decoded.id}', '${new Date().toISOString()}', '${new Date().toISOString()}', '1' )`
