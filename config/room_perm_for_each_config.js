...
function start(totnoofcomp, output, RoomPermutations) {
  ...
  for (var i = 0; i < output.length; i++) {
    if (output[i].length == totnoofcomp) {
      //totnoofcomp
      console.log("here", output[i]);
      output[i].sort(function (a, b) {
        return b - a;
      });
      a1.push(separate(RoomPermutations, output[i]));
    }
    // console.log(...a1)
  }
  ty = all();
...
function permutee(arrr, aaa) {
  //-->this fun will give separate vals
